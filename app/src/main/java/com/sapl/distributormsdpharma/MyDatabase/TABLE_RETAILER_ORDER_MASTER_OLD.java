...
public class TABLE_RETAILER_ORDER_MASTER_OLD {
    ...
    public static List<OrderDeliveryStatusModel> getNewOrderList() {
        ...
        if (true) {
            ...
            query = "SELECT count(d.ItemId) as count,  p.FIRMNAME  as shopName,  p.ContactPerson as mobNo, " +
                    ...
                    "  and o.orderId = d.OrderId ORDER BY o.orderId DESC ";
        }
    ...
    public static List<OrderDeliveryStatusModel> getOrderStatusList(String statusId, String filerSortColomnName) {
        List<OrderDeliveryStatusModel> orderStatusList = new ArrayList<>();
        SQLiteDatabase db = MyApplication.db.getWritableDatabase();
       ...
       /*String query1 = " SELECT count(d.ItemId) as count,  p.FIRMNAME  as shopName,  p.ContactPerson as mobNo, " +
        ...
        // HERE filterColomn may be ="orderID" OR "shop_name" OR "CALENADER" ,, OR " shop & calender "
        String query;
        // FOR THE ORDERS THAT ARE ACCEPTED OR REJECTED, DELIVERED
        ...
        query = "SELECT count(d.ItemId) as count,  p.FIRMNAME  as shopName,  p.ContactPerson as mobNo, " +
                ...
                " WHERE o." + COL_RETAILER_ID + " = cast(p.CustomerId as int)" +
                " and o."+COL_ORDER_ID+" = 'OS.RetailerOrderMasterID' "+
                " and o.OrderStatus = '" + statusId + "'  and o.orderId = d.OrderId ORDER BY o.orderId DESC ";
        ...
        if (c.getCount() > 0) {
            ...
            do {
               ...
               // String statusDate = TABLE_ORDER_STATUS.getStatusDate(orderId);
                ...
                String statusDate = c.getString(c.getColumnIndexOrThrow("statusDate"));
                String statusSttusID = c.getString(c.getColumnIndexOrThrow("statusID"));
        ...
        c.close();
        db.close();
        MyApplication.logi(LOG_TAG, " model is -->>" + orderStatusList.toString());
        return orderStatusList;
    }

    public static List<OrderDeliveryStatusModel> getOrderStatusListBy(String statusId) {
        List<OrderDeliveryStatusModel> orderStatusList = new ArrayList<>();
        SQLiteDatabase db = MyApplication.db.getWritableDatabase();
        MyApplication.logi(LOG_TAG, "in getOrderStatusList()");
       ...
       /*String query1 = " SELECT count(d.ItemId) as count,  p.FIRMNAME  as shopName,  p.ContactPerson as mobNo, " +
        ...
        if (statusId.equals("*")) {
            query = "SELECT count(d.ItemId) as count,  p.FIRMNAME  as shopName,  p.ContactPerson as mobNo, " +
                    ...
                    " WHERE o." + COL_RETAILER_ID + " = cast(p.CustomerId as int)" +
                    //" and o."+COL_ORDER_ID+" = 'OS.RetailerOrderMasterID' "+
                    "  and o.orderId = d.OrderId group by o.orderId DESC";
        } else {
            query = "SELECT count(d.ItemId) as count,  p.FIRMNAME  as shopName,  p.ContactPerson as mobNo, " +
                    " p.CustomerId as custId, o.* " +
                    //" OS.StatusDateTime as statusDate, OS.StatusID as statusID " +
                    ...
                    " WHERE o." + COL_RETAILER_ID + " = cast(p.CustomerId as int)" +
                    //" and o."+COL_ORDER_ID+" = 'OS.RetailerOrderMasterID' "+
                    " and o.OrderStatus = '" + statusId + "'  and o.orderId = d.OrderId group by o.orderId";
        }
   ...
   /* public static String CREATE_TABLE = "CREATE TABLE IF NOT EXISTS "
    ...
    public static int getPendingOrderCount() {
        ...
        String sql_get_count = "select count(OrderStatus) as count from " + TABLE_RETAILER_ORDER_MASTER_OLD.NAME + " where OrderStatus ='1'";
    ...
    public static void updateStatus(String appOrderid, String statusId) {
        ...
        cv.put("OrderStatus", statusId);
        long ret = db.update(TABLE_RETAILER_ORDER_MASTER_OLD.NAME, cv, "OrderId=?", new String[]{appOrderid});
        db.close();
